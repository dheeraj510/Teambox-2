...
// url                       (url to amazon bucket, default = 'http://' + options.host + '/javascripts/fancyupload/source/Swiff.Uploader.swf')
...
//
// For more options see the FanyUpload3 documentation:
...
FancyUpload3.S3Uploader = new Class({
	...
	initialize: function(list, selects, options) {
                ...
                options.url = options.url || 'https://' + options.bucket + '.s3.amazonaws.com/';
      ...
      onFileComplete: function(file) {
        if (file.response.code == 201 || file.response.code == 0){
          ...
          if (this.options.onUploadCompleteURL)
            new Request({ async: false,
                          method: this.options.onUploadCompleteMethod,
                          url: this.options.onUploadCompleteURL,
                          data: { 'upload_element_id' : file.ui.element.id, 'authenticity_token' : this.options.formAuthenticityToken, 'filename' : file.name, 'filesize' : file.size, 'filename_prefix' : file.prefix || '' } }).send();
        }
      },
